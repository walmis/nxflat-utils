...
typedef void
(*func_type) (asymbol *sym, void *arg1, void *arg2, void *arg3);
...
void copy_section (bfd *ibfd, sec_ptr isection, bfd *obfd)
{
  ...
  if (bfd_get_section_flags(ibfd, isection) & SEC_HAS_CONTENTS)
    {
      void *memhunk = 0;
      if (isection->vma >= DATA_OFFSET)
        {
          segment_info *info = &data_info;
          memhunk = info->contents + (isection->vma - DATA_OFFSET);

          bfd_set_section_contents(obfd, osection, memhunk, (file_ptr) 0, size);
          return;
        }
      else
        {
          for (int i = 0; i < text_info.nsubsects; i++)
            {
              asection *s = text_info.subsect[i];
              if (s->index == isection->index)
                {
                  memhunk = text_info.contents + (isection->vma);

                  bfd_set_section_contents(obfd, osection, memhunk,
                                           (file_ptr) 0, size);
                  return;
                }
            }
      ...
      memhunk = (void*) malloc((unsigned) size);
      bfd_get_section_contents(ibfd, isection, memhunk, (file_ptr) 0, size);
...
void copy_relocs (bfd *ibfd, sec_ptr isection, bfd *obfd)
{
  long relsize;
  long relcount;
  sec_ptr osection;
  ...
  if (relsize)
    {
      ...
      relcount = bfd_canonicalize_reloc(ibfd, isection, relpp,
                                        obfd->outsymbols);
      if (relcount)
        {
...
static void setup_section (bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  osection = bfd_make_section_with_flags(obfd, name, flags);

  if (osection == NULL)
    {
      err("failed to create output section\n");
      goto loser;
    }
...
//    elf_section_type (osection) = SHT_NOBITS;
  ...
  if (!bfd_copy_private_section_data(ibfd, isection, obfd, osection))
    {
      err("failed to copy private data\n");
      goto loser;
    }
...
void copy_object (bfd *ibfd, bfd *obfd)
{
  ...
  if (!bfd_set_start_address(obfd, start)
      || !bfd_set_file_flags(
          obfd, (bfd_get_file_flags(ibfd) & bfd_applicable_file_flags(obfd))))
    {
      fatal("set_file_flags");
    }
  ...
  bfd_set_section_size(obf, sect, got_size);
  bfd_set_section_vma(obf, sect, DATA_OFFSET);

  bfd_copy_private_header_data(ibfd, obfd);

  /* This has to happen before section positions are set.  */
  bfd_map_over_sections(ibfd, (void*) copy_relocs, obfd);

  /* This has to happen after the symbol table has been set.  */
  bfd_map_over_sections(ibfd, (void*) copy_section, obfd);

  bfd_set_section_contents(obf, sect, (void*) got, 0, got_size);
...
static void traverse_global_symbols (void *arg1, void *arg2, void *arg3,
                                     func_type fn)
{
  int i;
...
static void check_special_symbol (asymbol *sym, void *arg1, void *arg2,
                                  void *arg3)
{
...
/***********************************************************************
 * return dynamic symbol for imported var "name"
 ***********************************************************************/

static asymbol* find_import(const char* name) {
  asymbol* ret = 0;
  static const char * dyninfo = "__dyninfo";

  for(int i = 0; i < number_of_symbols; i++)
    {
      asymbol* sym = symbol_table[i];
      if(strncmp(sym->name, dyninfo, strlen(dyninfo)) == 0)
        {
          uint32_t text_offset = *(uint32_t*)(sym->section->contents + sym->value);
          char* import_name = text_info.contents+text_offset;
...
static void put_special_symbol (asymbol *begin_sym, asymbol *end_sym,
  ...
  if (begin_sym != NULL)
    {
      vdbg("begin: '%s' end: '%s' offset: %08lx\n", begin_sym->name,
           end_sym->name, (long)(NXFLAT_HDR_SIZE+offset));
      ...
      if (begin_sym->section == NULL)
      ...
      else
        {
          ...
          begin_sect_vma = begin_sym->section->vma - DATA_OFFSET;
      ...
      if (entry_point == 0)
      ...
      else
        {
          ...
          err("  Valid TEXT range: %08lx - %08lx\n", (long)NXFLAT_HDR_SIZE,
              (long)(NXFLAT_HDR_SIZE + text_info.size));
          exit(1);
        }
    }
...
#if GOT
static struct nxflat_got_s*
find_got_entry (asymbol *sym)
{
  int i;
  ...
  if (find_got_entry(sym) == 0)
    {
      ...
      newgot = (struct nxflat_got_s*) realloc(
          got_offsets, sizeof(struct nxflat_got_s) * noffsets);
      if (!newgot)
        {
...
static int add_nxflat_reloc (struct nxflat_reloc_s *reloc)
{
  ...
  for (i = 0; i < nxflat_nrelocs; i++)
    {
      if (nxflat_relocs[i].r_info == reloc->r_info)
        {
          return -1;
        }
    }

  relocs = (struct nxflat_reloc_s*) realloc(
      nxflat_relocs, sizeof(struct nxflat_reloc_s) * (nxflat_nrelocs + 1));
  if (!relocs)
    {
  ...
  nxflat_relocs[nxflat_nrelocs - 1].r_info = reloc->r_info;
  return 0;
}

static void relocate_abs32 (asection *sect, arelent *relp, int32_t *target,
                            symvalue sym_value)
{
  reloc_howto_type *how_to = relp->howto;
  asymbol *rel_sym = *relp->sym_ptr_ptr;
  ...
  *target = (long) temp;
  ...
  struct nxflat_reloc_s tmpreloc;
  tmpreloc.r_info = NXFLAT_RELOC(
      reloc_type, relp->address + (sect->vma - DATA_OFFSET) + got_size);

  add_nxflat_reloc(&tmpreloc);

  vdbg("relocs[%d]: type: %d offset: %08x\n", nxflat_nrelocs - 1,
       NXFLAT_RELOC_TYPE(tmpreloc.r_info),
       NXFLAT_RELOC_OFFSET(tmpreloc.r_info));
...
int is_thumb_func (asymbol *rel_sym)
{
  return ((((elf_symbol_type*) rel_sym)->internal_elf_sym.st_info & 0x0f)
      == STT_ARM_TFUNC
      || (((elf_symbol_type*) rel_sym)->internal_elf_sym.st_info & 0x0f)
          == STT_FUNC);
}

static void resolve_segment_relocs (bfd *input_bfd, segment_info *inf,
                                    asymbol **syms)
{
  arelent **relpp;
  int relsize;
  ...
  for (i = 0; i < inf->nsubsects; i++)
    {
      ...
      relcount = bfd_canonicalize_reloc(input_bfd, inf->subsect[i], relpp,
                                        syms);
      ...
      for (j = 0; j < relcount; j++)
        {
          ...
          int32_t *target;
          //uint32_t		   sym_vma = bfd_section_vma(bfd, rel_section);
          symvalue sym_value;

          if (relpp[j]->address >= DATA_OFFSET)
            {
              target = (int32_t*) (inf->contents + (int) relpp[j]->address
                  - DATA_OFFSET);
            }
          else
            {
              int32_t vma = inf->subsect[i]->vma;
              if (vma >= DATA_OFFSET)
                {
                  vma -= DATA_OFFSET;
                }
              target =
                  (int32_t*) (inf->contents + vma + (int) relpp[j]->address);
            }
...
#ifdef NXFLAT_THUMB2
          sym_value |= is_thumb_func(rel_sym);
           ...
           */
#if GOT
          if ((rel_section->flags & SEC_CODE) == 0
#if !GOT
	      && (rel_section->flags & SEC_READONLY) == 0
#endif
              && (rel_section->flags & SEC_ALLOC) != 0)
            {
              sym_value += got_size;
            }
#endif
          dbg("rel %-3d: sym [%20s] s_addr @ %08lx val %08lx-%08lx rel A:%08lx how %s\n",
              j, rel_sym->name,
              (int )(relpp[j]->address) + inf->subsect[i]->vma,
              (long )rel_sym->value, (long )sym_value, (long )relpp[j]->addend,
              how_to->name);
            ...
            case R_ARM_PC24:
              {
                ...
                dbg("performing PC24 link at addr %08lx [%08lx] to sym '%s' [%08lx]\n",
                    (long )relpp[j]->address, (long )*target, rel_sym->name,
                    (long )sym_value);
		...
		*target = (long)nxflat_swap32(temp);
            ...
            case R_ARM_TARGET2:
              ...
              int tmp = *target + sym_value + rel_section->vma;
              int place = inf->subsect[i]->vma + relpp[j]->address + got_size;
              //int place = (void*)target - (void*)inf->contents + got_size;
              *target = tmp - place;
            ...
            case R_ARM_ABS32:
              {
                dbg("Performing %s link at addr %08lx [%08lx] to sym '%s' [%08lx]\n",
                    how_to->name, (long )relpp[j]->address, (long )*target,
                    rel_sym->name, (long )sym_value);

                if (get_reloc_type(inf->subsect[i],
                NULL) == NXFLAT_RELOC_TARGET_TEXT)
                  {
                    err("%s reloc encountered in .text section, did you forgot to turn on -fpic ?\n",
                        how_to->name);
                    nerrors++;
                  }
                else
                  {
                    relocate_abs32(inf->subsect[i], relpp[j], target,
                                   sym_value);
                  }
              }
              break;
            ...
            case R_ARM_PREL31:
              {
                ...
                // A - addednd
                // P - place address
                //int32_t abs_address = (rel_section->vma + relpp[j]->address) + sym_value;
                int32_t abs_address = *target + sym_value + rel_section->vma;
                int starget = *target;
                if (!(*target & 0x80000000))
                  {
                    *target = abs_address;
                    /* put abs address - nxflat loader will convert this to PREL31 address */
                    //*(uint32_t*)target = abs_address ;
                    //*(uint32_t*)target = 0xDEADBEEF ;
                    struct nxflat_reloc_s reloc;
                    if (relpp[j]->address > DATA_OFFSET)
                      {
                        reloc.r_info = NXFLAT_RELOC(
                            NXFLAT_RELOC_TYPE_PREL31,
                            relpp[j]->address + (DATA_OFFSET) + got_size);
                      }
                    else
                      {
                        reloc.r_info =
                            NXFLAT_RELOC(
                                NXFLAT_RELOC_TYPE_PREL31,
                                relpp[j]->address + (inf->subsect[i]->vma - DATA_OFFSET) + got_size);
                      }

                    dbg("%s: offs %08x target:%08x abs:%08x val:%08x vma:%08x\n",
                        how_to->name, NXFLAT_RELOC_OFFSET(reloc.r_info),
                        starget, abs_address, (unsigned int )sym_value,
                        (unsigned int )rel_section->vma);

                    if (add_nxflat_reloc(&reloc) < 0)
                      {
                        //printf ("dup\n");
                      }
                  }
            ...
            case R_ARM_THM_JUMP24:
              /* Thumb BL (branch long instruction).  */
              {
                ...
                if (rel_sym->flags == BSF_WEAK)
                  {
                    dbg("Fixup undefined weak symbol %s as NOP\n",
                        rel_sym->name);
                    *target = 0x8000F3AF; /* replace unresolved weak sym with NOP */
                    //printf("Fixup ")
                    break;
                  }
                ...
                dbg("Performing THM link at addr %08lx [%04x %04x] to sym '%s' [%08lx]\n",
                    ...
                    rel_sym->name, (long )sym_value);
                dbg("  Original INSN: %04x %04x temp: %08lx\n", upper_insn,
                    lower_insn, (long )temp);
                ...
                signbit = (temp < 0) ? 1 : 0;
                upper_insn = (upper_insn & ~0x7ff) | ((temp >> 12) & 0x3ff)
                    | (signbit << 10);
                ...
                dbg("  Modified INSN: %04x %04x temp: %08lx Sec VMA: %08lx\n",
                    upper_insn, lower_insn, (long )temp,
                    (long )rel_section->vma);
                ...
                if (reltype == NXFLAT_RELOC_TARGET_TEXT)
                  {
                    ...
                    err("  At addr %08lx to sym '%s' [%08lx]\n",
                        (long )relpp[j]->address, rel_sym->name,
                        (long )sym_value);
                  }
                else
                  {
            ...
            case R_ARM_GOT_PREL:
              {
                ...
                dbg("Performing %s reloc at addr %08lx [%08lx] to sym '%s' [%08lx]\n",
                    relpp[j]->howto->name, (long )relpp[j]->address,
                ...
                if (!(rel_sym->flags & (BSF_LOCAL|BSF_GLOBAL|BSF_FUNCTION|BSF_WEAK)))
                  {
                    ...
                    //printf("%p\n", import);
                    /* add one word to import symbol, there lies the imported address */
                    dbg("  Original value: %08lx\n", (long )*target);
                    *target = import->value + 4 + got_size;
                    dbg("  Modified value: %08lx\n", (long )*target);
                  }
                ...
                else
                  {
                    ...
                    if (!got_entry)
                      {
                        err("No GOT entry from for symbol '%s'\n",
                            rel_sym->name);
                        nerrors++;
                      }
                    else
...
#endif
            default:
              dbg("Do not know how to handle reloc %d type %s @ %p!\n",
 ...
 */
#if GOT
static void allocate_segment_got (bfd *input_bfd, segment_info *inf,
                                  asymbol **syms)
{
  arelent **relpp;
  int relsize;
      ...
      for (j = 0; j < relcount; j++)
        {
          ...
          dbg("rel %-3d: sym [%20s] s_addr @ %08lx rel %08lx how %s\n", j,
              rel_sym->name, (long )relpp[j]->address, (long )relpp[j]->addend,
              how_to->name);
                ...
                if(rel_sym->flags & (BSF_LOCAL|BSF_GLOBAL|BSF_FUNCTION|BSF_WEAK))
                  {
                    ...
                    dbg("  Created GOT entry %d for sym %p (offset %d)\n",
                        ngot_offsets - 1, got_offsets[ngot_offsets - 1].sym,
                        got_offsets[ngot_offsets - 1].offset);
                  }
              }
              break;
  ...
  if (bfd_is_com_section(psym->section))
    {
      ...
      printf("Sym[%24s] @            sz %04lx ", psym->name,
             (long) psym->value);
      printf("align %04x ", (u_int32_t) isym->st_value);
    }
  else
    {
      printf("Sym[%24s] @ %04lx align            ", psym->name,
             (long) psym->value);
...
#ifdef NXFLAT_THUMB2
  if ((isym->st_info & 0x0f) == STT_ARM_TFUNC
      || (isym->st_info & 0x0f) == STT_ARM_16BIT
      || (isym->st_info & 0x0f) == STT_FUNC)
    {
      putchar('T');
    }
  else
      ...
      for (j = (i + 1); j < number_of_symbols; j++)
        {
          ...
          base_j = sym_j->symbol.section->vma
              + sym_j->internal_elf_sym.st_value;
...
static void map_common_symbols (bfd *input_bfd, asymbol **symbols,
      ...
      if (bfd_is_com_section(symbols[i]->section))
        {
          ...
          vdbg("  ba: %08lx sb: %08lx al: %04lx sz: %04lx of: %04lx\n",
               (long )baseaddr, (long )symbase, (long )align, (long )size,
               (long )offset);
              ...
              if (bss_s == symbols[j]->section)
                {
                  if (verbose)
                    {
                      message("Checking endsym? %08lx sym[%04d] ",
                              (long )baseaddr, j);
                  ...
                  if (symbols[j]->value >= baseaddr)
                    {
                      ...
                      ((elf_symbol_type*) symbols[j])->internal_elf_sym.st_value +=
                          offset;
                      if (verbose > 1)
                        {
      ...
      for (i = 0; i < ngot_offsets; i++)
        {
          ...
          if (rel_sym->flags == BSF_WEAK)
          ...
          else
            {
              ...
              switch (get_reloc_type(rel_section, NULL))
                ...
                case NXFLAT_RELOC_TARGET_DATA:
                  {
                    vdbg("Symbol '%s' lies in %s\n", rel_sym->name,
                         rel_section->name);
                    reloc_type = NXFLAT_RELOC_TYPE_REL32D;
                    sym_value += got_size + rel_section->vma;
                  }
                  break;
          ...
          vdbg("GOT[%d]: sym name: '%s' value: %08lx->%08lx\n", i,
               rel_sym->name, (long )rel_sym->value, (long )sym_value);

          /* And output the relocation information associate with the GOT entry */
          struct nxflat_reloc_s reloc;
          reloc.r_info = NXFLAT_RELOC(reloc_type, sizeof(u_int32_t) * i);
          add_nxflat_reloc(&reloc);
...
static void register_section (asection *s, segment_info *inf)
{
  ...
  if (inf->nsubsects >= inf->subsect_alloc)
    {
      inf->subsect_alloc += 32;
      inf->subsect = realloc(inf->subsect,
                             (inf->subsect_alloc) * sizeof(asection*));
  ...
  if (inf->size > 0)
    {
      ...
      memset(inf->contents, 0, inf->size);
      ptr = inf->contents;

      for (i = 0; i < inf->nsubsects; i++)
        {
          ptr = inf->contents + (inf->subsect[i]->vma - inf->low_mark);
          inf->subsect[i]->contents = ptr;
          if (!bfd_get_section_contents(bfd, inf->subsect[i], ptr, 0,
                                        bfd_section_size(inf->subsect[i])))
...
static void show_usage (void)
{
  ...
  DEFAULT_STACK_SIZE);
  fprintf(stderr, "  -v Verbose output. If -v is applied twice, additional\n");\
  fprintf(stderr,
          "     debug output is enabled [Default: no verbose output].\n");

  fprintf(
      stderr,
      "\n  -p --private-key\n     RSA private key file to sign the executable\n");
  fprintf(stderr,
          "  -H --hash-type=md4,md5,sha1\n     Hash type used for signing\n");
...
static void parse_args (int argc, char **argv)
{
  ...
  while ((opt = getopt_long(argc, argv, "hHpde:lo:s:v", long_options, NULL))
      != -1)
    {
      switch (opt)
        {
...
int main (int argc, char **argv, char **envp)
{
  ...
  text_info.subsect_alloc = MAX_SECTIONS;
  data_info.subsect_alloc = MAX_SECTIONS;
  bss_info.subsect_alloc = MAX_SECTIONS;
  text_info.subsect = malloc(sizeof(asection*) * MAX_SECTIONS);
  data_info.subsect = malloc(sizeof(asection*) * MAX_SECTIONS);
  bss_info.subsect = malloc(sizeof(asection*) * MAX_SECTIONS);

  memset(text_info.subsect, 0, sizeof(asection*) * MAX_SECTIONS);
  text_info.name = "text";
  data_info.name = "data";
  bss_info.name = "bss";
...
#if !GOT
	  || (((s->flags & SEC_ALLOC) != 0 && (s->flags & SEC_READONLY) != 0))
#endif
          )
            {
  ...
  if (text_info.nsubsects == 0)
    ...
    {
      printf("TEXT %08lx %08lx %08lx\n", (long) text_info.low_mark,
             (long) text_info.high_mark, (long) text_info.size);
  ...
  if (data_info.nsubsects == 0)
    ...
    {
      printf("DATA %08lx %08lx %08lx\n", (long) data_info.low_mark,
             (long) data_info.high_mark, (long) data_info.size);

      if (data_info.low_mark != DATA_OFFSET)
        {
          err("data section must be origined at 0x%08x", DATA_OFFSET);
          exit(1);
        }
    }
  ...
  if (bss_info.nsubsects == 0)
    ...
    {
      printf("BSS  %08lx %08lx %08lx\n", (long) bss_info.low_mark,
             (long) bss_info.high_mark, (long) bss_info.size);
...
#if !GOT
  if(!bfd_get_section_by_name(bf, ".got"))
    {
      err("cannot find .got section\n");
      exit(1);
    }
  else
    {
      ...
      for(i = 0; i < data_info.nsubsects; i++)
	{
	  if(strcmp(data_info.subsect[i]->name, ".got") == 0)
	    {
	      asection* sect = data_info.subsect[i];

	      uint32_t* start_addr = (uint32_t*)(data_info.contents + (sect->vma - DATA_OFFSET));
	      uint32_t* end_addr = (uint32_t*)((size_t)start_addr + (size_t)sect->size);
	      for(uint32_t* addr = start_addr; addr < end_addr; addr++)
		{
		  long offset = (long)addr-(long)start_addr;
		  printf("%x :%p -> %x\n", offset, addr, *addr);
		  struct nxflat_reloc_s reloc;

		  if(*addr & DATA_OFFSET)
		    {
		      reloc.r_info = NXFLAT_RELOC(NXFLAT_RELOC_TYPE_REL32D, offset);
		    }
		  else
		    {
		      reloc.r_info = NXFLAT_RELOC(NXFLAT_RELOC_TYPE_REL32I, offset);
		    }
		  add_nxflat_reloc(&reloc);
		}
  ...
  put_xflat16(&hdr.h_reloccount, nxflat_nrelocs + got_size / sizeof(uint32_t));
  ...
  fd = open(out_filename, O_RDWR | O_PLATFORM | O_CREAT | O_TRUNC, 0744);
  if (fd < 0)
    {
...
#endif
      nxflat_write(fd, (const char*) nxflat_relocs,
                   sizeof(struct nxflat_reloc_s) * nxflat_nrelocs);
    }

  bfd *ibf;
  if (!(ibf = bfd_openr(argv[argc - 1], 0)))
    {
      err("Failed to open %s\n", argv[argc - 1]);
      exit(1);
    }
  ...
  if (bfd_check_format(ibf, bfd_object) == 0)
    {
      err("File is not bfd_archive\n");
      exit(2);
    }
#if 0
  char* name = malloc (strlen (argv[argc - 1]) + 16);
  strcpy (name, argv[argc - 1]);
  strcat (name, "-dbg.elf");

  obf = bfd_openw (name, bfd_get_target(ibf));
  free (name);
